...
MakeAllNplus1Trees(Tree *TT)
{
  ...
  for (i = 0; i < Numtree(N); i++) {
    for (j = 1; j <= 2 * N - 3; j++) {
      ...
      /* copy tree i */
      TempTree  = PTAlloc(N + 1);
      T[k]      = TempTree[0];

      for (l = 1; l <= N; l++)
        T[k].L[l].e1 = TT[i].L[l].e1;
      ...
      if (tmpv > 0) {
        ...
        /* interior node */
        if (T[k].I[-tmpv].e1 == j)
          T[k].I[-tmpv].e1 = 2 * N - 3 + 1;
        else if (T[k].I[-tmpv].e2 == j)
          T[k].I[-tmpv].e2 = 2 * N - 3 + 1;
        else if (T[k].I[-tmpv].e3 == j)
          T[k].I[-tmpv].e3 = 2 * N - 3 + 1;
      }
...
MakeRandomTree(int n)
{
  ...
  for (N = 3; N < n; N++) {
    ...
    if (tmpv > 0) {
      ...
      /* interior node */
      if (TT->I[-tmpv].e1 == j)
        TT->I[-tmpv].e1 = 2 * N - 3 + 1;
      else if (TT->I[-tmpv].e2 == j)
        TT->I[-tmpv].e2 = 2 * N - 3 + 1;
      else if (TT->I[-tmpv].e3 == j)
        TT->I[-tmpv].e3 = 2 * N - 3 + 1;
    }
         ...
         int  which)
{
  ...
  if ((v1 > 0) || (v2 > 0))
    return NULL;                     /* null pointer if not interior edge */
       ...
       int  call)
{
  ...
  if (call) {
    /* dirty trick for initialization */
    int_edge    = n;
    int_vertex  = 1;
  }
