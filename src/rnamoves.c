 ...
 */
inline int isOutLonely( int i, int j)
{
 ...
 */
inline int isInsLonely( int i, int j)
{
...
void RNA2_move_noLP_bpdel() {
    ...
    for( i=0; i<r2d->len; i++)
        if( r2d->form[i] == '(')
        {
            ...
            {
                if( ilg && isOutLonely(i, j) ) /* only delete lonely stack on the inside */
                {
                    open_bp( i+1, j-1);
                    if(VERB)
...
void RNA2_move_bpins_to_right(int i, int avoid_end)
{
    ...
    for (j=i+1;j<r2d->len; j++) {
        ...
        if (r2d->form[j]==')') { /* here, the current loop ends */
            break;
        } else {
            /* j unpaired,
               register neighbor */
