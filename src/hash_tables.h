...
/* Bluntly taken from ViennaRNA Package with stripped vrna_ prefixes */
 ...
 *  @{
 *  @brief  Various implementations of hash table functions
 *
 ...
 *  - ht_db_comp() for the @p compare_function,
 *  - ht_db_hash_func() for the @p hash_function, and
 *  - ht_db_free_entry() for the @p free_hash_entry
 *
 *  arguments.
 *  @endparblock
 ...
 *
 *  @see ht_insert(), hash_delete(), ht_init()
 *
 *  @param  ht  The hash table
 ...
 *
 *  @see ht_init(), hash_delete(), ht_clear()
 *
 *  @param  ht  The hash table
 ...
 *  the bound @callback_ht_free_entry() function.
 *
 *  @see ht_free(), ht_init()
 *
 *  @param  ht  The hash table
 ...
 *  by calling the #callback_ht_free_entry() function for each
 ...
 *  @see  ht_init(), ht_db_comp(), ht_db_hash_func(), ht_db_free_entry()
 */
typedef struct {
 ...
 *  It assumes the both entries @p x and @p y are of type #ht_entry_db_t
 *  and compares the @p structure attribute of both entries
 *
 *  @see #ht_entry_db_t, ht_init(), ht_db_hash_func(), ht_db_free_entry()
 *
 *  @param  x   A hash table entry of type #ht_entry_db_t
 *  @param  y   A hash table entry of type #ht_entry_db_t
 ...
 *  assumes that entries are of type #ht_entry_db_t and uses
 *  the Bob Jenkins 1996 mix function to create a hash key from the
 *  @p structure attribute of the hash entry.
 *
 *  @see  #ht_entry_db_t, ht_init(), ht_db_comp(), ht_db_free_entry()
 *
 ...
 *
 *  This function assumes that hash entries are of type #ht_entry_db_t
 *  and free's the memory occupied by that entry.
 *
 *  @see  #ht_entry_db_t, ht_init(), ht_db_comp(), ht_db_hash_func()
 *
 *  @param  hash_entry  The hash entry to remove from memory
 *  @return             0 on success
 */
int ht_db_free_entry(void *hash_entry);


uint64_t
hash_function_uint64 (void          *hash_entry,
                      uint64_t hashtable_size);
