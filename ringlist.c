...
static void ini_or_reset_rl(char *seq,char *struc);
...
static void close_bp(rlItem *i,rlItem *j);
...
void RNA_init(char *seq, int shift, int nolp) {
  xtof = shift;
  noLP = nolp;
  farbe = seq;
}
...
/*      update_fold_params(); */
    ...
    for(i=0;i<len;i++){
      ...
      if(pos==NULL) rl[i].base=0;
      else rl[i].base=pos-Law_and_Order;
      rl[i].nummer=i;
      rl[i].next=&rl[i+1];
...
void close_bp(rlItem *i,rlItem *j){
...
static void make_poList(rlItem *root){
  ...
  for(rli=stop->next;rli!=stop;rli=rli->next){
    if(rli->typ=='p'){ /* wenn basenpaar untersuche subrlItem */
      poList[poListop++]=rli;
      make_poList(rli);
    }
  }
...
static void inb_noLP (rlItem *root) {
  ...
  for (rli = rlj = stop->next; rli!= stop; rli = rli->next) {
    /* helix elongation */
    if (rli->typ == 'p') {
      if (rli->prev == stop || rli->next == stop) continue;
      if (rli->prev->typ == 'p' || rli->next->typ == 'p') continue;
      if (pair[rli->prev->base][rli->next->base]) { /* enongate helix 2 the exterior */
	close_bp(rli->prev, rli->next);
	push(form);
	open_bp(rli->prev->up);
      }
      if (rli->down->next == rli->down || rli->down->prev == rli->down) continue;
      if (rli->down->next->typ == 'p' || rli->down->prev->typ == 'p') continue;
      if (pair[rli->down->next->base][rli->down->prev->base]) { /* enongate helix 2 the interior */
	...
	if (rlj->prev->nummer - rli->next->nummer < MYTURN) continue;
	/* cont if i or j pos of immediate bp is allready paired */
	if (rli->next->typ == 'p' || rlj->prev->typ == 'p') continue;
...
static void dnb_nolp(rlItem *rli) {
  ...
  rlj = rli->down;
  if (rlj->next == rlj->prev) { /* immediate interior bp ? */
    rlin = rlj->next;
    rljn = rlin->down;
  }
  if (rli->prev == rli->next && rli->next->typ != 'x') { /* immediate exterior */
    rlip = rli->next->up;
    rljp = rli->next;
  }
...
static void fnb(rlItem *rli){
  ...
  for(rlj=stop->next;rlj!=stop;rlj=rlj->next){
    if((rlj->typ=='p')||(rlj->typ=='q')) continue; /* prevent shifting to paired position */
    if((rlj->nummer-rli->nummer >= MYTURN)&&(pair[rli->base][rlj->base])){ /* (ij)->(ik) i<k<j */
    ...
    if((stop->nummer-rlj->nummer >= MYTURN)&&(pair[stop->base][rlj->base])){ /* (ij)->(kj) i<k<j */
      open_bp(rli); /* open original basepair */
